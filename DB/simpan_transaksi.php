...
if ($data) {
    ...
    $query_transaksi = "INSERT INTO transaksi (
        kode_transaksi, jenis_iklan, judul_iklan, lama_durasi, nama_pengiklan, tanggal_penayangan, total_harga, status
    ) VALUES (
        '$kode_transaksi', '$jenis_iklan', '$judul_iklan', $lama_durasi, '$nama_pengiklan', '$tanggal_penayangan', $total_harga, '$status'
    )";
    ...
    if ($koneksi->query($query_transaksi) === TRUE) {
        ...
        $query_data_iklan = "INSERT INTO data_iklan (
            nama_pengiklan, judul_iklan, lama_durasi, tanggal_penayangan, status_iklan
        ) VALUES (
            '$nama_pengiklan', '$judul_iklan', $lama_durasi, '$tanggal_penayangan', '$status'
        )";
